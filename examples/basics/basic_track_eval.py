...
Input: detections of all ground truth sequences from a model in "predictions" folder
Output: Evaluations of all trackers
Requirement: "trackeval" folder generated by "2_convert_to_mot_challenge_gt.py"
in "data-processing" repo.

Pitfalls:
1. While tracking, if "trackeval/trackers/.../{tracker_name}/{seq_name}.txt"
already exists, it will skip tracking that sequence.

Steps:
1. Load detection files from "predictions" folder
2. Track detections with multiple trackers
3. Save tracking results as "trackeval/trackers/.../{tracker_name}/{seq_name}.txt"
4. Evaluate tracking results using "TrackEval" package
"""
